...
- 이동: 양쪽 가장자리 버튼(이전/다음 화면 이름 표시), 상단 네비 탭, 키보드 ←/→, 마우스 휠, 모바일 스와이프, 하단 점 인디케이터, URL 해시(`#profile` 등).
- 전환: 나가는 화면은 반대편으로 밀리며 페이드, 들어오는 화면은 expo.out 으로 슬라이드 + 내부 요소(`data-part`) 순차 등장. 배경은 LivingWater 디졸브.
- 넘치는 화면(Experience 11개 카드)은 화면 안에서만 스크롤(`data-scroll`); 끝에 닿았을 때만 휠이 화면 전환으로 넘어간다.
