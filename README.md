...
## 화면 구조 — 가로 덱 (v0.7, 레퍼런스 space-planet)

세로 스크롤 대신 **5개 화면을 좌우로 넘기는** 구조. 한 화면 = 바다 하나.

| # | 화면 | 배경 장면 |
|---|---|---|
| 01 | Home (히어로) | Shallows |
| 02 | Profile — 학력 · 연구 · 자격증 · 관심 분야 | Shallows |
| 03 | Experience — Awards / Projects / Activities 서브 탭 | Lagoon |
| 04 | Vision | Coral Reef |
| 05 | Contact | Deep Blue |

- 이동: 양쪽 가장자리 버튼(이전/다음 화면 이름 표시), 상단 네비 탭, 키보드 ←/→, 모바일 가로 스와이프, 하단 점 인디케이터, URL 해시(`#profile` 등). **마우스 휠은 화면 안 스크롤에만 쓰이고 화면 전환에는 쓰지 않는다.**
- 전환: 나가는 화면은 반대편으로 밀리며 페이드, 들어오는 화면은 expo.out 으로 슬라이드 + 내부 요소(`data-part`) 순차 등장. 배경은 LivingWater 디졸브.
- 넘치는 화면(Experience 11개 카드)은 화면 안에서만 스크롤(`data-scroll`).
- 화면 순서·이름은 `App.tsx` 의 `SLIDES`, 배경 매핑은 `content.ts` 의 `sectionScenes`.
