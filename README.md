...
- **배경 영상**: Pexels 5277928 (돌고래 두 마리가 수면으로 올라가는 장면, 1080p, CORS 허용). 9초 지점부터 6초(180프레임, 720px)를 캡처해 부메랑으로 왕복 재생. 재생 속도는 `PLAYBACK_SPEED`(기본 0.45)로 조절. 모바일/저사양/reduced-motion 에서는 캡처 없이 `<video loop>` + 9초 시점 루프(같은 playbackRate).
